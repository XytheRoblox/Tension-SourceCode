...
public class ModuleButton {
	 ...
	 * Konstrukor
	 */
	public ModuleButton(ModuleMain imod, Panel pl) {
		mod = imod;
		 ...
		 */
		if (Tension.instance.setmgr.getSettingsByMod(imod) != null)
			for (Setting s : Tension.instance.setmgr.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Tension.instance.setmgr.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Tension.instance.clickgui.closeAllSettings();
				this.extended = b;
